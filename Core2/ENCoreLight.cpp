...
void ENCoreLight::CheckVisibility()
{
 //Get frustum planes
 ENEngineFrustum frust=ENCalculateFrustumPlanes();
 ...
 for(ENuint l=0;l<NumLights;l++)
 {
  ...
  Lights[l].Visible=ENSphereInFrustum(pos.x,pos.y,pos.z,range,&frust);
 }
}
...
void ENCoreLight::DeleteLight(ENuint ID)
{
 ...
 light=Lights[index];
 //Copy
 memmove(Lights+index,Lights+index+1,NumLights-index-1);
 NumLights--;
 //Delete light
