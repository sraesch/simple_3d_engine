 ...
 private:
   ...
   static void DrawPanel(ENScriptPanel *panel)
   {
    ...
    if(pic)
       ...
       glBindTexture(GL_TEXTURE_2D,0);
    //TexCoord
    if(!panel->MirrorX) t[0].x=0; else t[0].x=1;
    if(!panel->MirrorX) t[1].x=1; else t[1].x=0;
    if(!panel->MirrorX) t[2].x=1; else t[2].x=0;
    if(!panel->MirrorX) t[3].x=0; else t[3].x=1;
    if(!panel->MirrorY) t[0].y=1; else t[0].y=0;
    if(!panel->MirrorY) t[1].y=1; else t[1].y=0;
    if(!panel->MirrorY) t[2].y=0; else t[2].y=1;
    if(!panel->MirrorY) t[3].y=0; else t[3].y=1;
    //Set color
    ...
    glVertex2f(panel->Pos.x,panel->Pos.y);
    glTexCoord2fv((ENfloat*)&t[1]);
    glVertex2f(panel->Pos.x+panel->Size.x,panel->Pos.y);
    glTexCoord2fv((ENfloat*)&t[2]);
    glVertex2f(panel->Pos.x+panel->Size.x,panel->Pos.y+panel->Size.y);
    glTexCoord2fv((ENfloat*)&t[3]);
    glVertex2f(panel->Pos.x,panel->Pos.y+panel->Size.y);
    glEnd();
   }
};
...
class ENCoreTexts
{
 ...
 private:
   ...
   static void DrawText(ENScriptText *text)
   {
    ...
    t[2].x=1;
    t[3].x=0;
    if(!text->Mirror) t[0].y=0; else t[0].y=1;
    if(!text->Mirror) t[1].y=0; else t[1].y=1;
    if(!text->Mirror) t[2].y=1; else t[2].y=0;
    if(!text->Mirror) t[3].y=1; else t[3].y=0;
    //Get font
    ...
    glBegin(GL_QUADS);
    glTexCoord2fv((ENfloat*)&t[0]);
    glVertex2f(txtpos.x,txtpos.y);
    glTexCoord2fv((ENfloat*)&t[1]);
    glVertex2f(txtpos.x+text->Size.x,txtpos.y);
    glTexCoord2fv((ENfloat*)&t[2]);
    glVertex2f(txtpos.x+text->Size.x,txtpos.y+text->Size.y);
    glTexCoord2fv((ENfloat*)&t[3]);
    glVertex2f(txtpos.x,txtpos.y+text->Size.y);
    glEnd();
    txtpos.x+=text->Size.x;
    }
   }
};
