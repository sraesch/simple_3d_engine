...
void ENCoreWindow::Idle()
{
 ...
 //Get message
 msg.message=-1;
 while(PeekMessage(&msg,NULL,NULL,NULL,PM_REMOVE)&&msg.message!=WM_QUIT)
 {
  //Process message
...
void ENCoreWindow::ProcessMButtons(UINT uMsg)
{
 //Variables
 static ENint LastButton=-1;
 static ENint LastButtonStat=-1;
 //Process
 switch(uMsg)
 {
  case WM_LBUTTONDOWN:
    if(LastButton!=(ENint)0||LastButtonStat!=1)
    {
     LastButton=0;
     LastButtonStat=1;
     ENCoreInput::UpdateKey(LastButton+256,true);
    }
  break;
  case WM_MBUTTONDOWN:
    if(LastButton!=(ENint)1||LastButtonStat!=1)
    {
     LastButton=1;
     LastButtonStat=1;
     ENCoreInput::UpdateKey(LastButton+256,true);
  ...
  break;
  case WM_RBUTTONDOWN:
    if(LastButton!=(ENint)2||LastButtonStat!=1)
    {
     LastButton=2;
     LastButtonStat=1;
     ENCoreInput::UpdateKey(LastButton+256,true);
  ...
  break;
  case WM_LBUTTONUP:
    if(LastButton!=(ENint)0||LastButtonStat!=0)
    {
     LastButton=0;
     LastButtonStat=0;
     ENCoreInput::UpdateKey(LastButton+256,false);
    }
  break;
  case WM_MBUTTONUP:
    if(LastButton!=(ENint)1||LastButtonStat!=0)
    {
     LastButton=1;
     LastButtonStat=0;
     ENCoreInput::UpdateKey(LastButton+256,false);
  ...
  break;
  case WM_RBUTTONUP:
    if(LastButton!=(ENint)2||LastButtonStat!=0)
    {
     LastButton=2;
     LastButtonStat=0;
     ENCoreInput::UpdateKey(LastButton+256,false);
