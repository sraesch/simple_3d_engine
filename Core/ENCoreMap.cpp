...
void ENCoreMap::Load(char *FileName)
{
 ...
 ENCoreCollision::SetBspTree(mapobj->GetBspTree());
 //Update camera
 ENCoreView::SetCamera(mapobj->GetCamera());
}
...
ENbool ENCoreMap::isPortalVisible(ENuint ind,ENEngineFrustum *f)
{
 ...
 return ENBoxInFrustum(portal->min.v[0],portal->min.v[1],portal->min.v[2],
                       portal->max.v[0],portal->max.v[1],portal->max.v[2],
       	               f);
}
...
void ENCoreMap::SetLightFace(ENMapBase::ENMapFace *face,ENbool omni)
{
 ...
 {//Omni directional lighting shader's
  if(mapobj->SupportMaterialType(face->texind,ENMapBuildLoader::ENMATBUMP))
  {
   ...
   ENCoreShaders::SetGenericBumpMapOmni();
  }
  else
    ENCoreShaders::SetGenericLightMapOmni();
 }
 else
 {//Spot directional lighting shader's
  if(mapobj->SupportMaterialType(face->texind,ENMapBuildLoader::ENMATBUMP))
  {
   ...
   mapobj->SetMaterial(face->texind,ENMapBuildLoader::ENMATBUMP);
   ENGLExtension::ActiveTexture(GL_TEXTURE0_ARB);
   ENCoreShaders::SetGenericBumpMapSpot();
  }
  else
    ENCoreShaders::SetGenericLightMapSpot();
 }
}
...
ENbool ENCoreMap::SetSun()
{
 ENMapBase::ENMapSun sun;
 mapobj->GetSun(&sun);
 if(sun.Enable)
 {
  ENShaderGenericSun::SetSun(GL_LIGHT0,&sun);
  ENShaderGenericSunBump::SetSun(GL_LIGHT1,&sun);
  return true;
 }
 else
   return false;
...
ENbool ENCoreMap::isSun(ENubyte suncolor[])
{
 ENMapBase::ENMapSun sun;
 mapobj->GetSun(&sun);
 memcpy(suncolor,sun.SunColor,4);
 return sun.Enable;
}
...
void ENCoreMap::UpdateSounds()
{
 //Alloc space for sounds
 ENSoundSrc *mapsounds=new ENSoundSrc[header.NumSounds];
 //Get sounds
 mapobj->GetSounds(mapsounds);
 //Update sounds
 ENCoreSound::UpdateSounds(header.NumSounds,mapsounds);
 //Release sounds
 delete[] mapsounds;
}
