...
//---------------------------------------------------------------------------
ENBSPTree    *ENCoreCollision::bsptree=NULL;
...
{
 ENVector vNormal;
 if(ENBSPTree::SphereCollision(bsptree,sphere->pos,sphere->range,&vNormal))
 {
  ComputeReflectVector(vNormal);
  return true;
 }
 else
   return false;
...
void   ENCoreCollision::SetBspTree(ENBSPTree *tree)
{
 bsptree=tree;
}
