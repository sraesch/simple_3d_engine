...
ENbool ENShaderGenericLightMapSpot::InitShader()
{
 ...
 shaderEnable=ENGLExtension::GetUniformHandle(ProgramHandle,"EnableShadow");
 //Check uniforms
 if(shaderMap==-1||shaderEnable==-1) return false;
 //Finished
 return true;
}
...
void ENShaderGenericLightMapSpot::Activate()
{
 ...
 ENGLExtension::Uniform(shaderEnable,(ENint)ENCoreConfig::GetShadowEnable());
}
///////////////////////////////////////////////////////////
...
ENbool ENShaderGenericBumpMapSpot::InitShader()
{
 ...
 shaderEnable=ENGLExtension::GetUniformHandle(ProgramHandle,"EnableShadow");
 //Check uniforms
 if(shaderMap==-1||BumpTex==-1||NormalTex==-1||shaderEnable==-1) return false;
 //Finished
 return true;
}
...
ENbool ENShaderGenericLightModelSpot::InitShader()
{
 ...
 shaderEnable=ENGLExtension::GetUniformHandle(ProgramHandle,"EnableShadow");
 //Check uniforms
 if(shaderMap==-1||ModelMatrix==-1||shaderEnable==-1) return false;
 //Finished
 return true;
}
...
ENbool ENShaderGenericBumpModelOmni::InitShader()
{
 ...
 if(ModelMatrix==-1||RotMatrix==-1||shaderMap==-1||
    BumpTex==-1||NormalTex==-1||shaderEnable==-1) return false;
 //Finished
 return true;
}
...
void ENShaderGenericBumpModelOmni::SetModelMatrix(ENMatrix4X4 *mat,ENMatrix3X3 *rmat)
{
 if(Active)
 {
  ENGLExtension::Uniform(ModelMatrix,false,mat);
  //Set rotation Matrix
  ENGLExtension::Uniform(RotMatrix,false,rmat);
 }
}
///////////////////////////////////////////////////////////
...
void ENShaderGenericBumpModelSpot::SetModelMatrix(ENMatrix4X4 *mat,ENMatrix3X3 *rmat)
{
 if(Active)
 {
  ENGLExtension::Uniform(ModelMatrix,false,mat);
...
void ENCoreShaders::SetMatrix(ENMatrix4X4 *mat,ENMatrix3X3 *rmat)
{
 switch(CurrentShader)
 {
  case ENGENERICSPOTMODEL:
