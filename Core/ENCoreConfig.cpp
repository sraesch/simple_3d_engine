...
ENint  ENCoreConfig::ShadowEnable=1;
ENint  ENCoreConfig::MaxLights=2;
ENint  ENCoreConfig::MaxShadows=1;
...
ENbool ENCoreConfig::GetShadowEnable()
{
 return ShadowEnable;
}
...
ENbool ENCoreConfig::InterpConfig(ENFile handle)
{
 ...
 //Read shadow data
 if(!ReadData(FileBuffer)) return false;
 //Finished
 delete[] FileBuffer;
 return true;
}

ENbool ENCoreConfig::ReadData(char FileBuffer[])
{
 //Variables
 ENint shp;
 ...
 //Read shadow enable
 if(!ReadVal(FileBuffer,"shadow_enable",ShadowEnable)) return false;
 //Read max lights
 if(!ReadVal(FileBuffer,"max_lights",MaxLights)) return false;
 //Read max shadows
 if(!ReadVal(FileBuffer,"max_shadows",MaxShadows)) return false;
 //Finished
 return true;
}
