...
//---------------------------------------------------------------------------
ENbool                          ENCoreInput::KeyStats[259];
ENCoreInput::ENInputFunc        ENCoreInput::KeyFuncs[259];
...
{
 if(stat)
   KeyFuncs[key%259].down=func;
 else
   KeyFuncs[key%259].up  =func;
}
...
void ENCoreInput::UpdateCursorForce(ENint &x,ENint &y)
{
 ...
 //Calculate cursor force
 Force.v[0]=(ENfloat)(x-CursorX[0]);
 Force.v[1]=(ENfloat)(y-CursorY[0]);
 //Reduce cursor force
 Force=Force*0.1f;
 //Check ranges
...
void ENCoreInput::RunKeyFunc(ENuint key,ENbool pressed)
{
 ...
 if(func>-1)
   ENCoreProcessManager::AddBaseProcess(func,false);
 else
