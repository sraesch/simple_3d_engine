...
//---------------------------------------------------------------------------
ENint        ENCoreDepthBuffer::Size=128;
...
ENbool ENCoreDepthBuffer::Init(HDC vdc,HGLRC vrc,HWND hwnd)
{
 ...
 GLenum precision;
 //Get config data
 ENCoreConfig::GetShadowData(precision,Size);
 //Generate and set texture ID
 glGenTextures(1,&TexID);
 glBindTexture(GL_TEXTURE_2D,TexID);
 //Generate depth texture
 glTexImage2D(GL_TEXTURE_2D,0,precision,Size,Size,0,GL_DEPTH_COMPONENT,GL_UNSIGNED_BYTE,NULL);
...
void ENCoreDepthBuffer::End(ENbool toTexture)
{
 if(toTexture)
 {
  glBindTexture(GL_TEXTURE_2D,TexID);
